...
def write_2images(
    ...
    if comet_exp is not None:
        comet_exp.log_image(
            "%s/gen_a2b_%s.jpg" % (image_directory, postfix),
            name="%s/gen_a2b_%s.jpg" % (image_directory, postfix),
        )
        comet_exp.log_image(
            "%s/gen_b2a_%s.jpg" % (image_directory, postfix),
            name="%s/gen_b2a_%s.jpg" % (image_directory, postfix),
        )
...
def write_html(
    ...
    html_file.close()
    if comet_exp is not None:
        comet_exp.log_asset(filename)


def write_loss(iterations, trainer, train_writer, comet_exp=None):
    members = [
        attr
    ...
    for m in members:
        ...
        comet_exp.log_metric(m, getattr(trainer, m))
...
def load_vgg16(model_dir):
    ...
    raise NotImplementedError("load_lua deprecated so method removed in utils.py")
